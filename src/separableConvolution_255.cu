...
#define KERNEL_RADIUS 127
#define KERNEL_LENGTH (2 * KERNEL_RADIUS + 1)

__constant__ float c_Kernel[ KERNEL_LENGTH ];

void setConvolutionKernel_255( float *h_Kernel )
{
...
#define	COLUMNS_BLOCKDIM_X 1
#define	COLUMNS_BLOCKDIM_Y 128
#define	COLUMNS_RESULT_STEPS 4
#define	COLUMNS_HALO_STEPS 1

#define	DEPTH_BLOCKDIM_X 1
#define	DEPTH_BLOCKDIM_Z 128
#define	DEPTH_RESULT_STEPS 4
#define	DEPTH_HALO_STEPS 1

__global__ void convolutionX_255_Kernel( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
...
__global__ void convolutionY_255_Kernel( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
...
__global__ void convolutionZ_255_Kernel( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
...
void convolutionX_255( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
    ...
    convolutionX_255_Kernel<<<blocks, threads>>>( d_Dst, d_Src, imageW, imageH, imageD, outofbounds, outofboundsvalue );
}

void convolutionY_255( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
    ...
    convolutionY_255_Kernel<<<blocks, threads>>>( d_Dst, d_Src, imageW, imageH, imageD, outofbounds, outofboundsvalue );
}

void convolutionZ_255( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
    ...
    convolutionZ_255_Kernel<<<blocks, threads>>>( d_Dst, d_Src, imageW, imageH, imageD, outofbounds, outofboundsvalue );
}
...
extern "C" int convolve_255( float *image, float *kernelX, float *kernelY, float *kernelZ, int imageW, int imageH, int imageD, int convolveX, int convolveY, int convolveZ, int outofbounds, float outofboundsvalue, int devCUDA )
{
    ...
    if ( convolveX != 0 )
    {
		setConvolutionKernel_255( kernelX );
		fprintf(stderr, "CX" );
	    convolutionX_255( d_Output, d_Input, imageW, imageH, imageD, outofbounds, outofboundsvalue );
	    getLastCudaError("convolutionX() execution failed\n");
		in = 1;
    }
    ...
    if ( convolveY != 0 )
    {
    	setConvolutionKernel_255( kernelY );

		fprintf(stderr, "CY" );
    	if ( in == 0 )
    	{
    		convolutionY_255( d_Output, d_Input, imageW, imageH, imageD, outofbounds, outofboundsvalue );
    		in = 1;
    	}
    	else
    	{
    		convolutionY_255( d_Input, d_Output, imageW, imageH, imageD, outofbounds, outofboundsvalue );
    		in = 0;
    	}
	    getLastCudaError("convolutionY() execution failed\n");
    }
    ...
    if ( convolveZ != 0 )
    {
		setConvolutionKernel_255( kernelZ );

		fprintf(stderr, "CZ" );

		if ( in == 0 )
		{
			convolutionZ_255( d_Output, d_Input, imageW, imageH, imageD, outofbounds, outofboundsvalue );
			in = 1;
		}
		else
		{
			convolutionZ_255( d_Input, d_Output, imageW, imageH, imageD, outofbounds, outofboundsvalue );
			in = 0;
		}
	    getLastCudaError("convolutionZ() execution failed\n");
    }
