...
#define KERNEL_RADIUS 63
#define KERNEL_LENGTH (2 * KERNEL_RADIUS + 1)

__constant__ float c_Kernel[ KERNEL_LENGTH ];

void setConvolutionKernel_127( float *h_Kernel )
{
...
#define	COLUMNS_BLOCKDIM_X 2
#define	COLUMNS_BLOCKDIM_Y 64
#define	COLUMNS_RESULT_STEPS 8
#define	COLUMNS_HALO_STEPS 1

#define	DEPTH_BLOCKDIM_X 2
#define	DEPTH_BLOCKDIM_Z 64
#define	DEPTH_RESULT_STEPS 8
#define	DEPTH_HALO_STEPS 1

__global__ void convolutionX_127_Kernel( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
...
__global__ void convolutionY_127_Kernel( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
...
__global__ void convolutionZ_127_Kernel( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
...
void convolutionX_127( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
    ...
    convolutionX_127_Kernel<<<blocks, threads>>>( d_Dst, d_Src, imageW, imageH, imageD, outofbounds, outofboundsvalue );
}

void convolutionY_127( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
    ...
    convolutionY_127_Kernel<<<blocks, threads>>>( d_Dst, d_Src, imageW, imageH, imageD, outofbounds, outofboundsvalue );
}

void convolutionZ_127( float *d_Dst, float *d_Src, int imageW, int imageH, int imageD, int outofbounds, float outofboundsvalue )
{
    ...
    convolutionZ_127_Kernel<<<blocks, threads>>>( d_Dst, d_Src, imageW, imageH, imageD, outofbounds, outofboundsvalue );
}
...
extern "C" int convolve_127( float *image, float *kernelX, float *kernelY, float *kernelZ, int imageW, int imageH, int imageD, int convolveX, int convolveY, int convolveZ, int outofbounds, float outofboundsvalue, int devCUDA )
{
    ...
    if ( convolveX != 0 )
    {
		setConvolutionKernel_127( kernelX );
	    convolutionX_127( d_Output, d_Input, imageW, imageH, imageD, outofbounds, outofboundsvalue );
		in = 1;
    }

    if ( convolveY != 0 )
    {
    	setConvolutionKernel_127( kernelY );

    	if ( in == 0 )
    	{
    		convolutionY_127( d_Output, d_Input, imageW, imageH, imageD, outofbounds, outofboundsvalue );
    		in = 1;
    	}
    	else
    	{
    		convolutionY_127( d_Input, d_Output, imageW, imageH, imageD, outofbounds, outofboundsvalue );
    		in = 0;
    	}
    }
    ...
    if ( convolveZ != 0 )
    {
		setConvolutionKernel_127( kernelZ );

		if ( in == 0 )
		{
			convolutionZ_127( d_Output, d_Input, imageW, imageH, imageD, outofbounds, outofboundsvalue );
			in = 1;
		}
		else
		{
			convolutionZ_127( d_Input, d_Output, imageW, imageH, imageD, outofbounds, outofboundsvalue );
			in = 0;
		}
    }
